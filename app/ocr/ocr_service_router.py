...
# ocr_service/router.py  —  LOCAL version, PADDLEOCR ONLY (no VLM/Ollama)
#
# VLM/Ollama has been fully removed from this pipeline. Extraction now
# relies entirely on:
#     upload ──► run_ocr (PaddleOCR + passporteye)  ──► text (+ passport MRZ)
#            └─► deterministic extract_fields(text)  ──► ALL document types,
#                                                          including fuzzy
#                                                          types like offer
#                                                          letters, via the
#                                                          regex-based
...
#
# If you ever want VLM back, the old code called extract_with_vlm() from
# vlm_fallback.py inside a `if VLM_ENABLED and _needs_vlm(...)` block. That
# file still exists but is no longer imported or called anywhere in this
...
def _to_ocr_fields(fields: list[dict]) -> list[OCRField]:
    """Keep only the known keys, so stray keys can't break validation."""
    out = []
    for f in fields:
        out.append(OCRField(
            field_name=str(f["field_name"]),
...
async def run_extraction(
    ...
    # extractors. No VLM step — this is the final result.
    if ocr.passport_fields:
        doc_type = "passport"
        fields = ocr.passport_fields
    else:
