...
# ╚══════════════════════════════════════════════════════════════════════════╝
# 
# @router.post("/login/request-otp", response_model=LoginOTPRequestResponse)
# async def request_login_otp(
    # body: LoginOTPRequestSchema, db: DBSession
# ) -> LoginOTPRequestResponse:
    # """
    # Step 1 — user submits email OR phone. If a matching active account
    # exists, a 6-digit code is sent to that channel. Response is always
    # generic to avoid revealing whether the account exists.
    # """
    # result = await service_request_login_otp(db, identifier=body.identifier)
    # return LoginOTPRequestResponse(**result)
# 
# 
# @router.post("/login/verify-otp", response_model=TokenResponse)
# async def verify_login_otp(
    # body: LoginOTPVerifySchema, request: Request, response: Response, db: DBSession
# ) -> TokenResponse:
    # """
    # Step 2 — user submits the same identifier + the code they received.
    # On success, issues access/refresh tokens exactly like the old
    # password login did.
    # """
    # ip = request.client.host if request.client else None
    # ua = request.headers.get("user-agent")
# 
    # result = await service_verify_login_otp(
        # db,
