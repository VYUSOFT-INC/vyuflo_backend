...
@router.post("/verify-phone", response_model=TokenResponse, status_code=200)
async def verify_phone(
    ...
    result = await service_verify_phone(
        db,
        user_id = current_user.user_id,
        otp     = body.otp,
    )
...
@router.post("/resend-phone-otp", status_code=200)
async def resend_phone_otp(
    db: DBSession,
    current_user: Current_User,
):
    await service_resend_phone_otp(db, user_id=current_user.user_id)
...
async def save_attorney_profile(
    ...
    NOTE: firm_id is intentionally NOT passed through here. The service
    derives firm_id itself from law_firm_name via _get_or_create_law_firm().
    AttorneyProfileRequest has no firm_id field — passing body.firm_id
    previously raised AttributeError on every call.
    """
