...
# =============================================================================
# ACTIVE — Admin User Management schemas (spec §A2/§A3).
#
# Role values are kept NATIVE to this DB (hr | app_admin | employee | attorney)
# — the app_admin->admin / attorney->lawyer rename was intentionally skipped
# per instruction; the frontend team is handling that normalisation on their
...
class UserStatsResponse(BaseModel):
    """GET /admin/users/stats — spec §A3"""
    totalUsers: TrendBlock
    activeAccounts: TrendBlock
    pendingApproval: TrendBlock
    suspended: TrendBlock


class AdminUserItem(BaseModel):
    """One row in GET /admin/users — spec §A3"""
    id: str
    name: str
    email: str
...
class AdminUserUpdateRequest(BaseModel):
    """PUT /admin/users/{id} — spec §A2. All fields optional."""
...
class AdminBulkRoleRequest(BaseModel):
...
class AdminBulkRoleResultItem(BaseModel):
    id: str
    status: str          # "success" | "failed"
    reason: Optional[str] = None


class AdminBulkRoleResponse(BaseModel):
    processed: int
    succeeded: int
    failed: int
