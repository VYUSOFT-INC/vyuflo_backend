...
async def _build_case_summary(
    ...
    return CaseSummary(
        ...
        visa_label=visa_label,
        case_number=getattr(app, "receipt_number", None),
        priority_date=_iso(app.priority_date) if app.priority_date else None,
        filed_date=_iso(app.created_at) if app.created_at else None,
        current_stage=current_key,
        stages=stages,
        overall_progress=overall_progress,
...
def _build_action_items(
    documents: list,
    app: "Application | None",
    pending_intake_sessions: list | None = None,
) -> list[ActionItem]:
    """
    Generate actionable tasks the employee should complete.
    Sources: documents needing action, application stage requirements.
    ...
    # 1. Documents that need re-upload (rejected)
    #
    # FIXED: route was hardcoded to "/documents/upload", which is not a real
    # route anywhere in the app (the Dashboard's own "Upload Docs" quick
    # action uses /applications/{id}?tab=tasks or /documents — never this).
    # Clicking a rejected-document action item 404'd or fell through to a
    # catch-all. Now routes to the specific application's Required Tasks
    # tab using the document's own application_id, which is confirmed to
    # actually render TaskRow's Replace/Re-upload flow.
    for doc in documents:
        if doc.status == "rejected":
            counter += 1
            doc_app_id = getattr(doc, "application_id", None)
            items.append(ActionItem(
                id=f"act_doc_rej_{counter}",
                title=f"Re-upload {_doc_display_name(doc)}",
                description=doc.rejection_reason if hasattr(doc, "rejection_reason") and doc.rejection_reason else "This document was rejected — please upload a corrected version.",
                ...
                due_date=None,
                days_left=None,
                route=f"/applications/{doc_app_id}?tab=tasks" if doc_app_id else "/documents",
                completed=False,
            ))
    ...
    if app and not app.is_draft:
        ...
        # If in document_collection and docs are missing, prompt upload
        #
        # FIXED: same broken "/documents/upload" route as above — this one
        # has a real application in scope (`app`), so it can point straight
        # at that case's Required Tasks tab instead of a dead route.
        if stage == "document_collection":
            counter += 1
            items.append(ActionItem(
                id=f"act_stage_{counter}",
                ...
                due_date=None,
                days_left=None,
                route=f"/applications/{app.id}?tab=tasks",
                completed=False,
            ))
    ...
    # 4. Pending intake sessions — surface as an urgent action item
    for s in (pending_intake_sessions or []):
        is_correction = s.review_status == "changes_requested"
        counter += 1
        items.append(ActionItem(
            id=f"act_intake_{s.id}",
            title=("Corrections needed on your intake" if is_correction
                   else "Complete your intake"),
            description=(s.review_note or
                         "Your attorney has requested your intake details."),
            category="form",
            priority="urgent",
            due_date=None,
            days_left=None,
...
async def service_get_dashboard(
    ...
    pending_intake_result = await db.execute(
        select(ClientIntakeSession)
        .join(Application, Application.id == ClientIntakeSession.application_id)
        .where(
            Application.user_id == user_id,
            or_(
                ClientIntakeSession.review_status == "changes_requested",
                and_(ClientIntakeSession.token.isnot(None),
                     ClientIntakeSession.is_submitted == False),
            ),
        )
    )
    pending_intake_sessions = pending_intake_result.scalars().all()
