...
from app.models.visamodels import Document, DocumentOCRField, DocumentType, ApplicationTask, Application
...
async def upload_document(
    ...
    # 5b. NEW — notify the case's assigned attorney that a document arrived
    if application_id:
        app_row = await db.scalar(
            select(Application).where(Application.id == application_id)
        )
        if app_row and app_row.assigned_attorney_id:
            from app.services.employee.notification_service import _create_notification
            notif = await _create_notification(
                db, user_id=app_row.assigned_attorney_id,
                notification_type="document_uploaded",
                category="case_update", priority="medium",
                title=f"New document uploaded — {document_type}",
                body=f"A '{document_type}' document was uploaded for case {app_row.application_number}.",
                application_id=application_id, document_id=doc.id,
                case_reference=app_row.application_number,
                actor_id=user_id, actor_label=None,
                cta_primary_label="Review", cta_primary_url=f"/lawyer/documents/{doc.id}/review",
            )
